...
def google_login():
    ...
    # User context check - Reverted to your more robust version from the snippet
    if not hasattr(request, 'current_user') or not request.current_user or not hasattr(request.current_user, 'id'): # type: ignore
        app.logger.error(f"--- google_login: User context not available. hasattr(request, 'current_user'): {hasattr(request, 'current_user')}, request.current_user: {getattr(request, 'current_user', 'N/A')}")
        error_resp = make_response(jsonify({"success": False, "error": "User context not available for initiating Google OAuth."}), 401)
        return error_resp

    user_id = request.current_user.id # type: ignore
    ...
    # app.logger.error(f"--- google_login: Client config for GoogleFlow: {client_config} ---") # Optional
    ...
    try:
        ...
        try:
            if flask_session is not None and flask_session.modified:
                ...
                if current_cookies_on_resp:
                    ...
                    app.logger.error(f"  NO cookies on THIS RESPONSE (id={id(resp)}) after manual save_session (unexpected if save_session ran & session modified).")
            else:
                app.logger.error("  Skipping manual save_session in google_login: session None or not modified (unexpected here).")
                if flask_session is None: app.logger.error("  Reason: flask_session is None.")
                elif not flask_session.modified: app.logger.error("  Reason: flask_session.modified is False.") # type: ignore
        ...
        app.logger.error(f"--- google_login: Returning response object (id={id(resp)}). Check @after_request logs. ---")
        return resp
