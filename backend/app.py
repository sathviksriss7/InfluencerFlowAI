...
def google_login():
    ...
    # CORRECTED: Ensure current_user is available from request object
    # Also check that request.current_user itself is not None before trying to access .id
    if not hasattr(request, 'current_user') or not request.current_user or not hasattr(request.current_user, 'id'):
        app.logger.error("--- google_login: User context not available from request.current_user, or current_user is None, or missing 'id'.")
        return jsonify({"success": False, "error": "User context not available for initiating Google OAuth."}), 401

    user_id = request.current_user.id # CORRECTED: Using request.current_user.id
